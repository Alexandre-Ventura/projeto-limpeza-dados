...
def converte_para_string(conjunto_dados, colunas):
...
# Converte colunas especificadas para inteiro
def converte_para_inteiro(conjunto_dados, colunas):
...
# Converte colunas especificadas para float
def converte_para_float(conjunto_dados, colunas):
...
# Converte colunas especificadas para datetime
def converte_para_datetime(conjunto_dados, colunas):
...
def multiplica_por_fator(conjunto_dados, colunas, fator):
    """Multiplica colunas especificadas por um fator.
    Parâmetros:
...
def calcula_porcentagem_valores_ausentes(dados):
    """Calcula a porcentagem de valores ausentes no conjunto de dados.
...
def calcula_porcentagem_ausentes_por_linha(dados):
...
def relatorio_valores_ausentes_por_coluna(dados):
    ...
    qntd_valores_ausentes = dados.isnull().sum()
    # Calcula a porcentagem de valores ausentes por coluna
    porcentagem_valores_ausentes = 100 * qntd_valores_ausentes / len(dados)
    ...
    resumo_valores_ausentes = pd.concat([qntd_valores_ausentes, porcentagem_valores_ausentes, tipos_dados], axis=1)
